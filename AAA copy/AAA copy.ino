...
Rstepper.setCurrentPosition(0);
  ...
  //move Right
  Xstepper.moveTo(Xstepper.currentPosition() + 1050);
  ...
  //move forward
  Ystepper.moveTo(Ystepper.currentPosition() - 452);
  while (Ystepper.distanceToGo() < 0) {Ystepper.run();}
  
  digitalWrite(Valvepin, HIGH);
  //open the valve then rotate the nozzle 
  if (Rstepper.currentPosition() ==0) {Rstepper.moveTo(200);
  while (Rstepper.distanceToGo() > 0) {Rstepper.run();}
  ...
  Ystepper.moveTo(Ystepper.currentPosition() + 452); //move back
  while (Ystepper.distanceToGo() > 0) {Ystepper.run();}
 //move up
 Zstepper.moveTo(300);
  while (Zstepper.distanceToGo() < 0) {Zstepper.run();}
...
while (col < 7) {
 ...
 //move Right
  Xstepper.moveTo(Xstepper.currentPosition() + 1139);
  while (Xstepper.distanceToGo() > 0) {Xstepper.run();}
...
Ystepper.moveTo( Ystepper.currentPosition() - 2140); 
    while (Ystepper.distanceToGo() < 0) {
    Ystepper.run();
    }
    delay (10);
